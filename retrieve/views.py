...
def search(request):
    ...
    results = (
        Chunk.objects.filter(embedding__isnull=False)  # <-- skip un-embedded chunks
    ...
    playload = {"query": query, "results": [], "message": ""}

    for i in distance_values:
        if (
            i["min_distance"] is not None and i["min_distance"] < 0.6
        ):  # <-- belt and suspenders
            bookmark = (
                Bookmark.objects.filter(id=i["bookmark_id"])
                .values("id", "title", "url", "platform", "author", "saved_at")
                .first()
            )
            tags = list(
                BookmarkTag.objects.filter(bookmark_id=i["bookmark_id"]).values_list(
                    "tag__name", flat=True
                )
            )
